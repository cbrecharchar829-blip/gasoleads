...
export default function MapView() {
  ...
  useEffect(() => {
    ...
    // Geocode all leads with location data (any company)
      ...
      try {
        const results = await Promise.all(
          withAddr.map(async (lead) => {
            // Extract coords from Google Maps URL — check for @ or direct coordinates
            if (lead.maps_url) {
              // Format: @lat,lng (Google Maps share link)
              const match1 = lead.maps_url.match(/@(-?\d+\.\d+),(-?\d+\.\d+)/);
              if (match1) return { ...lead, lat: parseFloat(match1[1]), lng: parseFloat(match1[2]) };

              // Fallback: try to extract from query parameter if present
              const match2 = lead.maps_url.match(/[?&]query=([^&]+)/);
              if (match2) {
                // This is a search URL, geocode the query
            ...
            // Nominatim geocoding — address + zipcode
  ...
  const buildAppleMapsUrl = () => {
    ...
    // Apple Maps supports multi-stop via lat/lng
                ...
                <Popup maxWidth={240}>
                  <div className="text-sm space-y-1.5">
                    ...
                    <div className="flex items-center gap-1.5 text-xs">
                      <span className={`w-2 h-2 rounded-full inline-block ${lead.color_status === 'red' ? 'bg-red-500' : lead.color_status === 'yellow' ? 'bg-yellow-400' : 'bg-green-500'}`} />
